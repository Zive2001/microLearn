...
const connectDB = async () => {
  try {
    ...
    console.warn(
      "⚠️ Database-dependent features will not work until connection is restored"
    );
  }
};
...
const checkDatabaseConnection = (req, res, next) => {
  // Skip database check for certain routes
  const skipDbCheck = ["/api/health", "/api/tts-test"];

  const shouldSkip = skipDbCheck.some((path) => req.path.startsWith(path));

  if (req.path.startsWith("/api") && !shouldSkip) {
